...
def main():
    st.set_page_config(page_title='Modulo19 Tarefa01',
                       page_icon='📖',
                       layout='wide')
    ...
    st.image(image)
    st.header('Telemarketing Analysis 🔎')
    st.markdown("---")

    #Upload de arquivo CSV OU EXCEL
    data_file_1 = st.file_uploader("Bank marketing data",
                                    type=['csv', 'xlsx'])
    st.write('---')
    ...
    if (data_file_1 is not None):
        ...
        st.write('### Proporção de aceite em Gráficos')
        # PLOTS
